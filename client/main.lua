...
function OpenCloakroomMenu()
	...
	if PlayerData.job ~= nil and (PlayerData.job.grade_name == 'recruit' or PlayerData.job.grade_name == 'agent' or PlayerData.job.grade_name == 'specialagent') then
		table.insert(elements, {label = 'Tenue Agent du PJ', value = 'pjb_wear'})
	end
	
	if PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = 'Tenue Directeur PJ ', value = 'pjboss_wear'})	
	end
	...
	if Config.EnableNonFreemodePeds then
		...
		ESX.UI.Menu.Open(
			...
			if data.current.value == 'pj_wear' and PlayerData.job.name == 'pj' then
			...
			if data.current.value == 'pjbboss_wear' and PlayerData.job.grade_name == 'boss' then
		...
		if data.current.value == 'swat_wear' and PlayerData.job.name == 'pj' then
...
function OpenArmoryMenu(station)
  ...
  if Config.EnableArmoryManagement then
    ...
    local elements = {
	  {label = 'Pegue um colete à prova de balas',  value = 'gilet_wear'},
	  {label = 'Remova o colete à prova de balas',  value = 'veste_wear'},
      {label = _U('get_weapon'), value = 'get_weapon'},
      {label = _U('put_weapon'), value = 'put_weapon'},
      {label = 'Pegar objeto',  value = 'get_stock'},
      {label = 'Depositar',  value = 'put_stock'}
    }
    ...
    for i=1, #Config.PjStations[station].AuthorizedWeapons, 1 do
      local weapon = Config.PjStations[station].AuthorizedWeapons[i]
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        TriggerServerEvent('esx_pj:giveWeapon', weapon,  1000)
      end,
      function(data, menu)
...
function OpenVehicleSpawnerMenu(station, partNum)
  ...
  local vehicles = Config.PjStations[station].Vehicles
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(garageVehicles)
      ...
      ESX.UI.Menu.Open(
        ...
        function(data, menu)
          ...
          TriggerServerEvent('esx_society:removeVehicleFromGarage', 'pj', vehicleProps)
	...
	table.insert(elements, { label = 'Buffalo PJ', value = 'fbi'})
	table.insert(elements, { label = 'Suv PJ', value = 'fbi2'})	
...
function OpenVehicle1SpawnerMenu(station, partNum)
	...
	local vehicles1 = Config.PjStations[station].Vehicles1
	...
	if Config.EnableSocietyOwnedVehicles1 then
		...
		for i=1, #Config.PjStations[station].AuthorizedVehicles1, 1 do
			local vehicle = Config.PjStations[station].AuthorizedVehicles1[i]
...
function OpenPjActionsMenu()

  ESX.UI.Menu.CloseAll()

  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'pj_actions',
    {
      title    = 'Pj',
      align    = 'top-left',
      elements = {
    ...
    function(data, menu)
      ...
      if data.current.value == 'citizen_interaction' then
          ...
          function(data2, menu2)
            ...
            if distance ~= -1 and distance <= 3.0 then
              ...
              if data2.current.value == 'handcuff' then
                TriggerServerEvent('esx_pj:handcuff', GetPlayerServerId(player))
              end

              if data2.current.value == 'drag' then
                TriggerServerEvent('esx_pj:drag', GetPlayerServerId(player))
              end

              if data2.current.value == 'put_in_vehicle' then
                TriggerServerEvent('esx_pj:putInVehicle', GetPlayerServerId(player))
              end

              if data2.current.value == 'out_the_vehicle' then
                  TriggerServerEvent('esx_pj:OutVehicle', GetPlayerServerId(player))
              end
					...
					function(data, menu)
						...
						if amount == nil then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pj', 'Pj', amount)
							end
						end
					end,
				function(data, menu)
        ...
        if data2.current.value == 'code' then
                TriggerServerEvent('esx_pj:codedmv', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrive' then
                TriggerServerEvent('esx_pj:codedrive', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrivebike' then
                TriggerServerEvent('esx_pj:codedrivebike', GetPlayerServerId(player))
              end
        
        if data2.current.value == 'codedrivetruck' then
                TriggerServerEvent('esx_pj:codedrivetruck', GetPlayerServerId(player))
              end 
        
        if data2.current.value == 'weaponlicense' then
                TriggerServerEvent('esx_pj:weaponlicense', GetPlayerServerId(player))
              end
...
function OpenIdentityCardMenu(player)
  ...
  if Config.EnableESXIdentity then
    ...
    ESX.TriggerServerCallback('esx_pj:getOtherPlayerData', function(data)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          TriggerServerEvent('esx_pj:confiscatePlayerItem', GetPlayerServerId(player), itemType, itemName, amount)
...
function OpenFineCategoryMenu(player, category)
  ...
  ESX.TriggerServerCallback('esx_pj:getFineList', function(fines)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_pj', _U('fine_total') .. label, amount)
        else
...
function OpenVehicleInfosMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_pj:getVehicleInfos', function(infos)
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_pj:getArmoryWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_pj:removeArmoryWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_pj:addArmoryWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
  ...
  ESX.TriggerServerCallback('esx_pj:getArmoryWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_pj:buy', function(hasEnoughMoney)
...
function OpenGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_pj:getStockItems', function(items)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_pj:getStockItem', itemName, count)
            end
...
function OpenPutStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_pj:getPlayerInventory', function(inventory)
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              TriggerServerEvent('esx_pj:putStockItems', itemName, count)
            end
...
AddEventHandler('esx_pj:hasEnteredMarker', function(station, part, partNum)
...
AddEventHandler('esx_pj:hasExitedMarker', function(station, part, partNum)
  ESX.UI.Menu.CloseAll()
  CurrentAction = nil
end)

AddEventHandler('esx_pj:hasEnteredEntityZone', function(entity)

  local playerPed = GetPlayerPed(-1)

  if PlayerData.job ~= nil and PlayerData.job.name == 'pj' and not IsPedInAnyVehicle(playerPed, false) then
...
AddEventHandler('esx_pj:hasExitedEntityZone', function(entity)
...
RegisterNetEvent('esx_pj:handcuff')
AddEventHandler('esx_pj:handcuff', function()
...
RegisterNetEvent('esx_pj:drag')
AddEventHandler('esx_pj:drag', function(cop)
...
RegisterNetEvent('esx_pj:putInVehicle')
AddEventHandler('esx_pj:putInVehicle', function()
...
RegisterNetEvent('esx_pj:OutVehicle')
AddEventHandler('esx_pj:OutVehicle', function(t)
...
Citizen.CreateThread(function()
  ...
  for k,v in pairs(Config.PjStations) do
...
Citizen.CreateThread(function()
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'pj' then
      ...
      for k,v in pairs(Config.PjStations) do
        ...
        if Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'pj' and PlayerData.job.grade_name == 'boss' then
  ...
  while true do
    ...
    if PlayerData.job ~= nil and PlayerData.job.name == 'pj' then
      ...
      if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastStation ~= currentStation or LastPart ~= currentPart or LastPartNum ~= currentPartNum) ) then
        ...
        then
          TriggerEvent('esx_pj:hasExitedMarker', LastStation, LastPart, LastPartNum)
          hasExited = true
        end
        ...
        TriggerEvent('esx_pj:hasEnteredMarker', currentStation, currentPart, currentPartNum)
      end
  ...
  while true do
    ...
    if closestDistance ~= -1 and closestDistance <= 3.0 then
      ...
      if LastEntity ~= closestEntity then
        TriggerEvent('esx_pj:hasEnteredEntityZone', closestEntity)
      ...
      if LastEntity ~= nil then
        TriggerEvent('esx_pj:hasExitedEntityZone', LastEntity)
        LastEntity = nil
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'pj' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            TriggerServerEvent('esx_society:putVehicleInGarage', 'pj', vehicleProps)
            ...
            then
              TriggerServerEvent('esx_service:disableService', 'pj')
            end
        ...
        if CurrentAction == 'menu_boss_actions' then
          ...
          TriggerEvent('esx_society:openBossMenu', 'pj', function(data, menu)
	...
	if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'pj' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'pj_actions') and (GetGameTimer() - GUI.Time) > 150 then
      OpenPjActionsMenu()
      GUI.Time = GetGameTimer()
    end
...
local TeleportFromTo = {
	...
	["PJ"] = {
		positionFrom = { ['x'] = 136.18350219727, ['y'] = -761.86651611328, ['z'] = 242.15200805664, nom = "Descer Para a Recessão"},
		positionTo = { ['x'] = 138.98336791992, ['y'] = -762.68725585938, ['z'] = 45.752006530762, nom = "Subir para o Escritório"},
	},
	
	["GARAGE"] = {
		positionFrom = { ['x'] = 136.0496673584, ['y'] = -761.77819824219, ['z'] = 45.752021789551, nom = "Descer Para A Garagem"},
		positionTo = { ['x'] = 144.23973083496, ['y'] = -689.15106201172, ['z'] = 33.128112792969, nom = "Subir Para a Recessão"},
	},

	["HELICO"] = {
		positionFrom = { ['x'] = 138.22854614258, ['y'] = -764.94403076172, ['z'] = 242.15211486816, nom = "Subir Para O Heliporto"},
		positionTo = { ['x'] = -67.257919311523, ['y'] = -821.841796875, ['z'] = 321.28735351563, nom = "Descer Para O Escritório"}, 
	},	
			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 150.0) then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 5.0)then
					...
					if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 1.0)then
						ClearPrints()
						DisplayHelpText("Carrega ~INPUT_PICKUP~ Pra ~b~".. j.positionFrom.nom,1, 1, 0.5, 0.8, 0.9, 255, 255, 255, 255)
			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 150.0) then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 5.0)then
					...
					if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 1.0)then
						ClearPrints()
						DisplayHelpText("Carrega ~INPUT_PICKUP~ Pra ~r~".. j.positionTo.nom,1, 1, 0.5, 0.8, 0.9, 255, 255, 255, 255)
						DrawSubtitleTimed(2000, 1)
						if IsControlJustPressed(1, 38) then
							DoScreenFadeOut(1000)
							Citizen.Wait(2000)
